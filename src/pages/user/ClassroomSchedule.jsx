    ...
    <>
      <div className='relative min-h-screen'>
        ...
        <div className='flex w-11/12 mx-auto justify-evenly font-bold '>
          ...
          <select name="room" id="room-select" className='bg-[#d3edf9] border-[3px] border-[#6fa9e3] text-[#6fa9e3] rounded-md px-4 py-1' value={room} onChange={e => setRoom(e.target.value)}>
            <option value="">선택</option>
            {rooms.map(room => (
              <option key={room} value={room}>{room}</option>
            ))}
          </select>
          <input type='submit' value='조회' className="bg-[#6fa9e3] text-[#d3edf9] px-4 rounded-md cursor-pointer" onClick={handleSearch} />
        </div>
        {imageSrc ? (
          <img src={imageSrc} alt="강의실 시간표 사진" className='w-11/12 border-2 rounded-md border-[#6fa9e3] mx-auto my-8' />
        ) : (
